...
namespace TreeTaxation
{
    public class MainWindowViewModel : INotifyPropertyChanged
    {
        ...
        public MainWindowViewModel()
        {
            ...
            FindPoints = new Point3DCollection();

            var reader = new LasReader("C:\\Users\\Vitsin\\source\\repos\\Las_Converter_Console\\01_ALS.las");

            var points = reader.ReadPoints().OrderBy(x => x.X).ThenBy(x => x.Y).ThenBy(x => x.Z).ToList();
        ...
        private async void DBSCANExecute()
        {
            ...
            await Task.Run(() =>
            {
                ...
                treeClusters = DBSCAN_KDTree.Cluster(_treePoints, eps: double.Parse(Eps), minPts: int.Parse(MinPts));
            ...
            {
                a.AddRange(cluster.Select(x => new Point3D(x.X / 0.01, x.Y / 0.01, x.Z / 0.01)));
            }

            FindPoints = new Point3DCollection(a);
